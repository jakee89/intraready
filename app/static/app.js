...
function lineRow(line, allLines, issueLines) {
  ...
  return `<tr id="line-${line.id}" class="${rowClass}" data-line-row="${line.id}" data-kind="${line.line_kind}" data-reviewed="${line.reviewed}" data-has-issue="${issueLines.has(line.id)}">
    ...
    <td><input data-line-id="${line.id}" data-line-field="hs_code" value="${escapeHtml(line.hs_code)}" maxlength="8" inputmode="numeric" ${line.line_kind!=="goods"?"disabled":""}><small class="subline" title="Full code printed by supplier">${escapeHtml(line.raw_commodity_code)}</small></td>
