    ...
    <div class="review-layout">
      ...
      <div class="review-workspace">
        ...
        <article class="panel review-section full-width-lines"><div class="section-title"><div><h3>Goods and charges</h3><p>Internal reference details remain here and stay out of XML.</p></div><div class="line-toolbar"><select id="lineFilter"><option value="all">All rows</option><option value="issues">Rows with issues</option><option value="unreviewed">Unreviewed</option><option value="goods">Goods only</option><option value="charges">Charges only</option></select><button id="saveReviewedButton" class="button secondary" ${state.pendingReviews.size ? "" : "disabled"}>Save reviewed rows${state.pendingReviews.size ? ` (${state.pendingReviews.size})` : ""}</button><button id="combineRowsButton" class="button quiet" title="Combine rows only when their Intrastat classification fields match">Combine equivalent</button><button id="addLineSmallButton" class="button secondary">Add</button></div></div>
          ...
          <div class="data-table-wrap"><table class="data-table line-table"><thead><tr><th>Type</th><th>SKU / description</th><th>Qty</th><th>CN code</th><th>Origin</th><th>Consignment</th><th>Invoice €</th><th>Stat €</th><th>Unit kg</th><th>Total kg</th><th>Supp. quantity</th><th>Other required fields</th><th>Reviewed</th><th>Actions</th></tr></thead><tbody id="lineTableBody">
          ${invoice.lines.map(line => lineRow(line, invoice.lines, issueLines)).join("") || `<tr><td class="empty-row" colspan="14">No rows were extracted. Add the first goods row manually.</td></tr>`}
          </tbody></table></div>
        </article>
    </div>`;
}
...
function lineRow(line, allLines, issueLines) {
  ...
  const supplementaryUnit = line.cn_reference?.supp_unit || line.supp_unit || "";
  const specialRequired = ["84151010", "84151090", "84191900", "84158100", "84158200"].includes(line.hs_code);
...
async function buildDeclarationPreview() {
  ...
  try {
    ...
    const previousDelta = data.totals.invoice_value - data.previous.totals.invoice_value;
    const breakdown = (title, items, label) => `<article class="panel declaration-breakdown"><h3>${title}</h3><div class="data-table-wrap"><table class="data-table"><thead><tr><th>${label}</th><th>Rows</th><th>Invoice €</th><th>Net kg</th></tr></thead><tbody>${items.length ? items.map(item => `<tr><td><strong>${escapeHtml(item.key)}</strong></td><td>${item.rows}</td><td class="amount">€${item.invoice_value.toLocaleString()}</td><td class="amount">${item.net_mass.toLocaleString()}</td></tr>`).join("") : `<tr><td colspan="4" class="empty-row">No included rows</td></tr>`}</tbody></table></div></article>`;
    $("#declarationContent").innerHTML = `
      <div class="declaration-summary"><div class="summary-card"><span>Ready invoices</span><strong>${data.invoice_count}</strong></div><div class="summary-card ${data.excluded_invoices.length ? "attention" : ""}"><span>Excluded</span><strong>${data.excluded_invoices.length}</strong></div><div class="summary-card"><span>Goods rows</span><strong>${data.row_count}</strong></div><div class="summary-card"><span>Invoice value</span><strong>€${data.totals.invoice_value.toLocaleString()}</strong></div><div class="summary-card"><span>Statistical value</span><strong>€${data.totals.statistical_value.toLocaleString()}</strong></div><div class="summary-card"><span>Net mass</span><strong>${data.totals.net_mass.toLocaleString()} kg</strong></div></div>
      <article class="info-card slim declaration-status"><span class="info-icon">${data.schema.valid ? "✓" : "!"}</span><div><strong>${data.schema.valid ? "XML schema ready" : "XML export needs attention"}</strong><p>${escapeHtml(data.schema.message)} Previous month (${escapeHtml(data.previous.period)}): ${data.previous.invoice_count} invoices and €${data.previous.totals.invoice_value.toLocaleString()}; current difference ${previousDelta >= 0 ? "+" : "−"}€${Math.abs(previousDelta).toLocaleString()}.</p></div></article>
      <div class="declaration-lists">
        <article class="panel declaration-list"><div class="panel-head"><div><p class="eyebrow">INCLUDED</p><h3>Ready for this export</h3></div></div>${data.included_invoices.length ? data.included_invoices.map(invoice => `<button data-open-invoice="${invoice.id}"><span><strong>${escapeHtml(invoice.invoice_number || "Invoice")}</strong><small>${escapeHtml(invoice.supplier || "Supplier not identified")}</small></span><span class="declaration-list-total">${invoice.row_count} rows · €${invoice.invoice_value.toLocaleString()}</span></button>`).join("") : `<div class="empty-state compact"><p>No approved invoices are ready for this month.</p></div>`}</article>
        <article class="panel declaration-list ${data.excluded_invoices.length ? "has-attention" : ""}"><div class="panel-head"><div><p class="eyebrow">NEEDS ATTENTION</p><h3>Excluded from this export</h3></div></div>${data.excluded_invoices.length ? data.excluded_invoices.map(invoice => `<button data-open-invoice="${invoice.id}"><span><strong>${escapeHtml(invoice.invoice_number || "Invoice")}</strong><small>${escapeHtml(invoice.supplier || "Supplier not identified")}</small></span><span class="declaration-reason">${escapeHtml(invoice.reasons[0])}${invoice.reasons.length > 1 ? ` · +${invoice.reasons.length - 1} more` : ""}</span></button>`).join("") : `<div class="empty-state compact"><p>Nothing assigned to this month is being left behind.</p></div>`}</article>
      </div>
      <div class="declaration-breakdowns">${breakdown("By commodity code", data.groups.by_cn, "CN code")}${breakdown("By consignment country", data.groups.by_country, "Country")}${breakdown("By supplier", data.groups.by_supplier, "Supplier")}</div>
      ...
      <div class="export-actions"><p>CSV includes internal references. XML contains official fields only. ${data.export_attempts.length ? `${data.export_attempts.length} export attempt${data.export_attempts.length === 1 ? "" : "s"} already saved for this month.` : "No export has been saved for this month yet."}</p><button id="downloadCsv" class="button quiet" ${data.row_count?"":"disabled"}>Download review CSV</button><button id="downloadXml" class="button primary" ${data.row_count && data.schema.valid?"":"disabled"}>Download validated XML</button></div></article>`;
  } catch (error) {
...
document.addEventListener("click", async event => {
  ...
  if (rereadLocal) {
    if (!confirm("Rebuild all invoice rows from the PDF using the corrected local supplier reader? Existing row edits and reviewed marks on this invoice will be replaced. No API credits are used.")) return;
    rereadLocal.disabled = true; rereadLocal.textContent = "Reading…";
    try { state.currentInvoice = await api(`/api/invoices/${state.currentInvoice.id}/extract-again?use_ai=false&replace_reviewed=true`, { method: "POST" }); await refreshBootstrap(); renderReview(); toast("Invoice rebuilt locally — no API credits used"); }
    catch (error) { rereadLocal.disabled = false; rereadLocal.textContent = "Re-read locally"; toast(error.message, "error"); }
    return;
  }
